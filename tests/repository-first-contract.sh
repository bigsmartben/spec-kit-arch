...
if search "name: speckit\\.arch\\.(generate|reverse)$" extension.yml >/dev/null; then
    echo "legacy broad arch commands must not be registered" >&2
    exit 1
fi

search "Commands count: 10" CATALOG-SUBMISSION.md >/dev/null
search "/speckit\\.arch\\.scenario\\.generate|/speckit\\.arch\\.physical\\.reverse" README.md >/dev/null
search 'legacy `/speckit\.arch`, `/speckit\.arch\.generate`, or `/speckit\.arch\.reverse`' README.md >/dev/null
